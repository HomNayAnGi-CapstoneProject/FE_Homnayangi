...
<div className='flex'>
  ...
  <p className='text-redError font-semibold'>{Intl.NumberFormat().format(food.price)}đ</p></> }
    ...
    <button className='flex justify-center items-center rounded-[10px] mr-5 p-2 min-w-[25%] btn-card hover:shadow-xl'><p className='mr-2 '>Công Thức</p> <div className="w-[24px] h-[24px] bg-cover" style={{ backgroundImage: `url(${ic_boiling_white})` }} /></button>
    <button className='flex justify-center items-center rounded-[10px] p-2 min-w-[20%] btn-order hover:shadow-xl'><p className='mr-2'>Đặt Làm</p><div className="w-[25px] h-[25px] bg-cover" style={{ backgroundImage: `url(${ic_add_to_cart_white})` }}/></button>
    </div>
    </div>
    </div>
    <div className="flex lg:hidden gap-[8px] py-2 px-2">
        <button className="bg-[#FF7940] flex-1 rounded-[10px] cursor-pointer text-white font-medium xs:text-[18px] xs:px-[20px] px-2 py-2 flex justify-center items-center gap-2">
          Công thức
