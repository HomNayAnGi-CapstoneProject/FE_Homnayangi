    ...
    <>
{
  foods.map((food) => {
    ...
    return(
      ...
      <div key={food.id} className=" flex card overflow-hidden">
...
<div className='flex'>
  <p className='font-inter font-extralight mr-1 '>{food.ingredient}:</p>
  <p className='text-redError font-semibold'>{food.price}</p>
</div>
    <p>{food.title}</p>
    <div className='grid grid-cols-4 mt-9'>
    <button className='flex justify-center items-center rounded-md mr-5 p-2 min-w-[20%] btn-card hover:shadow-xl'><p className='mr-2'>Công Thức</p> <div className="w-[24px] h-[24px] bg-cover" style={{ backgroundImage: `url(${ic_boiling_white})` }} /></button>
