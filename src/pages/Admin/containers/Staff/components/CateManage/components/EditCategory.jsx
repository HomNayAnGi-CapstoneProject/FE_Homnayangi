  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)}>
      ...
      {errors?.name?.type === 'pattern' && (
        <p className="mb-[5px] text-redError text-[14px]">Tên danh mục không hợp lệ</p>
      )}
...
const EditCategory = () => {
  ...
  useEffect(() => {
    const fetch = async () => {
      const res = await instances.get(`/categories/${params.categoryId}`);
  ...
  return (
    <div>
      <p className="font-semibold text-[18px]">Chỉnh sửa danh mục</p>
      <div className="bg-white py-5 px-5 rounded-[5px] mt-4">
        {categoryData ? <EditForm data={categoryData} /> : 'Loading...'}
      </div>
    </div>
  );
};
